...
const SectionHeading = ({
  ...
  <div className={`${align === 'center' ? 'premium-section-heading' : 'max-w-3xl'} ${light ? 'text-white' : ''}`}>
    <p className={`premium-kicker ${align === 'center' ? 'justify-center' : ''} ${light ? 'text-primary' : 'text-primary'}`}>
      {eyebrow}
    </p>
    ...
    {description && (
      <p className={`mt-5 max-w-2xl text-sm leading-7 sm:text-base ${align === 'center' ? 'mx-auto' : ''} ${light ? 'text-white/52' : 'text-muted-foreground'}`}>
        {description}
      </p>
    )}
  </div>
    ...
    <div className="container mx-auto px-5 sm:px-6">
      ...
      <div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-3 lg:gap-5">
        {HOME_NEED_CARDS.map((card, index) => {
          ...
          const inner = (
            <>
              <div className="flex items-start justify-between gap-5">
                ...
                <span className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-ink/24">0{index + 1}</span>
              </div>
              <div className="mt-8">
          ...
          const classes = `premium-card group flex min-h-[290px] flex-col rounded-[1.7rem] border border-white/80 bg-white/78 p-6 backdrop-blur-sm sm:p-7 ${index === 0 || index === 5 ? 'lg:min-h-[330px]' : ''}`;
      ...
      <div className="mt-8 flex justify-center">
        <p className="inline-flex items-center gap-2 rounded-full border border-ink/8 bg-white/60 px-4 py-2 text-xs font-semibold text-ink/50">
...
export const PremiumHowItWorksSection = () => (
  <section className="premium-section overflow-hidden bg-ink">
    ...
    <div className="absolute -right-40 bottom-0 h-[36rem] w-[36rem] rounded-full bg-secondary/12 blur-3xl" aria-hidden />
    ...
    <div className="container relative z-10 mx-auto grid gap-14 px-5 sm:px-6 lg:grid-cols-[0.82fr_1.18fr] lg:gap-20">
      ...
      <ol className="relative space-y-4">
        <div className="absolute bottom-10 left-[1.65rem] top-10 w-px bg-gradient-to-b from-primary/60 via-white/12 to-transparent" aria-hidden />
        {HOW_STEPS.map((step, index) => (
          <li key={step.title} className="premium-glass group relative rounded-[1.7rem] p-5 sm:p-6">
            <div className="flex gap-5">
              <span className={`relative z-10 flex h-[3.3rem] w-[3.3rem] shrink-0 items-center justify-center rounded-full border ${index === 0 ? 'border-primary/30 bg-primary text-white' : 'border-white/12 bg-ink text-white/70'}`}>
              ...
              <div className="pt-1">
                ...
                <p className="mt-2 max-w-xl text-sm leading-7 text-white/48">{step.text}</p>
              </div>
            </div>
          </li>
        ))}
    ...
    <div className="container mx-auto px-5 sm:px-6">
      ...
      <div className="mt-12 grid gap-4 md:grid-cols-2">
        {CLUB_EVENTS.map((event, index) => (
          <article key={event.title} className="premium-card group rounded-[1.8rem] border border-white/80 bg-white/82 p-6 sm:p-8">
            ...
            <div className="mt-7 flex items-center gap-2 text-xs font-semibold text-ink/42">
        ...
        {CLUB_RESOURCES.map((resource, index) => (
          ...
          >
            <div className="flex items-center justify-between">
              <span className="inline-flex items-center gap-2 rounded-full border border-ink/8 bg-muted/70 px-3 py-1.5 text-[10px] font-extrabold uppercase tracking-[0.15em] text-ink/45">
                <BookOpen className="h-3.5 w-3.5 text-primary" /> {resource.tag}
              </span>
              <ArrowRight className="h-4 w-4 text-ink/22 transition-all duration-300 group-hover:translate-x-1 group-hover:text-primary" />
            </div>
