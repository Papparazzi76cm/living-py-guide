...
const EMPTY: PartnerApplication = {
  name: '', company: '', email: '', whatsapp: '', city: '', category: '', website: '',
  description: '', yearsExperience: '', languages: '', exclusivityInterest: 'no',
};
...
export const PartnerApplicationForm = ({ defaultCategory = '' }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(false);
    setSubmitted(form);
    toast({ title: 'Simulación completada', description: 'El formulario funciona en modo MVP. La persistencia se conectará en la siguiente fase.' });
  };
    ...
    return (
      <div className="rounded-3xl border border-secondary/25 bg-secondary/5 p-8 text-center sm:p-12">
        ...
        <p className="mx-auto mt-3 max-w-md text-sm leading-relaxed text-muted-foreground">
          La experiencia de postulación para <strong className="text-ink">{submitted.category || 'esta categoría'}</strong> funciona correctamente. En este MVP los datos no se guardan ni se envían todavía; conectaremos la candidatura real al backend y al CRM en la siguiente fase.
        </p>
        <button type="button" onClick={() => { setSubmitted(null); setForm({ ...EMPTY, category: defaultCategory }); }} className="mt-7 rounded-xl border border-border bg-card px-5 py-3 text-sm font-semibold text-ink transition-colors hover:bg-muted">
          Probar otra postulación
        </button>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-3xl border border-border bg-card p-6 sm:p-10">
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <div>
          ...
          <select id="category" required className={inputClass} value={form.category} onChange={(e) => set('category', e.target.value)}>
            ...
            {PARTNER_CATEGORIES.map((c) => {
              ...
              return <option key={c.slug} value={c.name} disabled={exclusive}>{c.name}{exclusive ? ' — categoría exclusiva (demo)' : ''}</option>;
            })}
          </select>
        </div>
        <div><label className={labelClass} htmlFor="website">Web o redes</label><input id="website" className={inputClass} value={form.website} onChange={(e) => set('website', e.target.value)} placeholder="https://" /></div>
        <div><label className={labelClass} htmlFor="years">Años de experiencia *</label><input id="years" type="number" min="0" required className={inputClass} value={form.yearsExperience} onChange={(e) => set('yearsExperience', e.target.value)} placeholder="8" /></div>
        ...
        <div className="sm:col-span-2"><label className={labelClass} htmlFor="description">Descripción breve de tu servicio *</label><textarea id="description" required rows={4} className={inputClass} value={form.description} onChange={(e) => set('description', e.target.value)} placeholder="Qué resolvés para un extranjero que llega a Paraguay y por qué sos la mejor opción de tu categoría." /></div>
        <fieldset className="sm:col-span-2">
          <legend className={labelClass}>¿Te interesa la exclusividad de categoría (USD 5.000/año adicionales)?</legend>
          <div className="mt-2 flex gap-3">
            {(['si', 'no'] as const).map((v) => (
              <label key={v} className={`flex-1 cursor-pointer rounded-xl border px-4 py-3 text-center text-sm font-semibold transition-colors ${form.exclusivityInterest === v ? 'border-primary bg-primary/10 text-primary' : 'border-border bg-background text-muted-foreground hover:bg-muted'}`}>
                <input type="radio" name="exclusivity" className="sr-only" checked={form.exclusivityInterest === v} onChange={() => set('exclusivityInterest', v)} />
      ...
      <button type="submit" disabled={submitting} className="mt-8 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-6 py-4 font-semibold text-primary-foreground transition-colors hover:bg-primary-hover disabled:opacity-60 sm:w-auto">
        {submitting && <Loader2 className="h-5 w-5 animate-spin" />} Probar postulación
      </button>
      <p className="mt-4 text-xs leading-relaxed text-muted-foreground">MVP: no se realiza ningún pago ni se persisten datos todavía.</p>
    </form>
  );
};
