...
export function ProviderForm({ provider, onSaved }: { provider: Provider | null; onSaved: () => void }) {
  ...
  return (
    <form onSubmit={save} className="space-y-5">
      <div><h2 className="text-2xl font-bold text-ink">Tu perfil profesional</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">El escaparate público mostrará tu presentación profesional. Tus datos directos de contacto se conservan para verificación y gestión de operaciones; no sustituirán el chat del marketplace.</p></div>
      ...
      <div className="grid gap-4 sm:grid-cols-2"><Field label="Web profesional (opcional)" name="provider-website" type="url" value={website} onChange={setWebsite} maxLength={500} placeholder="https://…" /><Field label="WhatsApp profesional (privado)" name="provider-whatsapp" value={whatsapp} onChange={setWhatsapp} maxLength={50} placeholder="+595…" /></div>
      ...
      <p className="text-xs leading-5 text-muted-foreground">Los cambios relevantes en un perfil aprobado vuelven a revisión para mantener la confianza del marketplace.</p>
